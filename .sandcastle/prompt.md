...
9. **Deploy new proxy rules to PREVIEW (only if `/api/*` changed)** — use the **`j5s-dev` BFFless MCP** to make the new/changed rules live on the **preview** alias. **Never** touch the production alias here.
   ...
   - For each rule the story adds, `create_proxy_rule` into that set, copying `pipelineConfig` **verbatim** from the rule source (Handoff's exported `*.proxy-rules.json`, or Studio's authored `rule.yaml`/`*.fn.js` under `.bffless/proxy-rules/<set>/`). **Reuse existing schema IDs** (e.g. `studio_jobs`, `studio_source`) exactly as the source lists them — do not invent schemas.
