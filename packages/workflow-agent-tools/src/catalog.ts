...
const DESCRIPTIONS: Readonly<Record<ToolName, string>> = {
  ...
  'workflow.submitStep':
    'Complete a waiting interactive step. A `form` step takes a value per field; an `island` step takes its declared outputs. Validated by the same checks a person’s submit runs; a refusal names each bad value.',
