...
export function checkStudioCommon(rec: RunRecord, r: Checker): void {
  const run = rec.run
  const failedSteps = rec.steps.filter((s) => s.status !== 'succeeded' && s.status !== 'skipped').map((s) => `${s.key}:${s.status}`)
  ...
  const firstBad = scenes.find((s) => !(typeof get(s, ['source']) === 'string' && typeof get(s, ['sourceIndex']) === 'number' && Array.isArray(get(s, ['spans'])))) ?? null
  r.expect('R.scenesCarrySourceSpans', scenes.length > 0 && scenes.every((s) => typeof get(s, ['source']) === 'string' && typeof get(s, ['sourceIndex']) === 'number' && Array.isArray(get(s, ['spans']))), { scenes: scenes.length, first: scenes[0] ?? null, firstBad })
