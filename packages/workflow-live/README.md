...
| `headless` | Task 25 Step 2: `driver.exit0`, `driver.wroteRunJson` (failure-branch only), the five `checkHeadlessHello` checks (`run.succeeded`, `run.headlessFlag`, `D7.islandSelfSubmitted`, `D11.reviewSkippedWithOutputs`, `run.posterIsFileRef`), `driver.savedPoster`, `driver.wrongTypeIsExit3`, and — with `--dispatch` — the same five re-scoped as `dispatch.*` plus `dispatch.jobGreen`, `dispatch.artifactHasRunJson` (failure-branch only), `dispatch.savedPoster` | nothing (CI minutes with `--dispatch`) |
