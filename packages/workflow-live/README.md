...
| `mcp` | the M5 Phase-2 walk (spec 10, D19/D22/D23; apps#554 stories 5–6): the MCP endpoint over the official SDK's stateless Streamable HTTP — `D22.getIs405`, `D22.initialize`, `D19.toolsListParity` (the catalog byte for byte), `spec10.appOnlyHidden`, `D19.listsHello`, `D20.describeInteractive`, `spec10.runsRequiresImpl`, `spec10.statusRequiresRunId`, `spec10.outputsOfRun`, `D6.signIsPresigned`, `spec10.notServedHonest` (`workflow.cancel` — the one catalog tool still not served over the endpoint → `errors.tool`, "not served by the MCP endpoint"; `workflow.start` is no longer probed because since ADR-0006 it dispatches a real driver job), `spec10.resourcesList` (CSP derived from the instance), `D22.unknownMethod`; then the Story 6 island round trip — `spec10.parkIsland` (a `hello/interactive` run parked on its island **through the page tools**, the browser then closed), `spec10.leaseLapses`, `spec10.resourcesReadIsland`, `spec10.cspDerived`, `spec10.stepViewMounts`, `spec10.pipelineFenced`, `spec10.annotateWrites`, `spec10.submitRefusesBad`, `spec10.submitWrites`, `record.stepSucceeded` (with `ADMIN_API_KEY`; the snapshot stands in without), `spec10.submitTwiceRefused` — then (Phase 3 story 7) `D23.bearerIsMember` (anonymous `initialize` → 401; `workflow.runs` shows the parked run `startedBy` the signed-in member) and `D23.readOnlyCannotSubmit` (a `workflow:read`-only token's `workflow.submit` → `errors.scope` naming `workflow:run`; its `workflow.status` still answers) — see `src/walks/mcp.ts`. The endpoint runs as the caller: the walk signs in through the relay, mints two app tokens through that browser context (`admin.<domain>/api/app-tokens`, 1 day, revoked at the end) and sends the first as `Authorization: Bearer`; `WORKFLOW_APP_TOKEN` skips the mint. The member credentials are needed for the mint and the park. `--run <id>` skips the park; `--park-only` parks, prints the run id and stops (how a person hands a fresh run to claude.ai). Not part of `all` | one hello run, left `running` with its island submitted |
| `oauth` | the M5 Phase-3 walk (spec 10, D23 rung 3; apps#554 story 9): claude.ai's connector flow, headless — `D23.prmServed` (the harness's `/.well-known/oauth-protected-resource` — CE derives it from the `oauth_protected_resource` step, so this asserts `resource`, a non-empty issuer, `scopes_supported` equal to the catalog's `SCOPES` as a set, and `bearer_methods_supported` carrying `header` — and CE's RFC 8414 metadata), `D23.anon401Hints` (an anonymous `initialize` → 401 with `resource_metadata`), `D23.dcr` (RFC 7591 registration), `D23.consentGrants` (the signed-in member's browser lands on `/oauth/consent`, three checkboxes, Allow → the local callback with the state), `D23.tokenIsAppToken` (`bfat_…`, 3600 s, a refresh token), `D23.statusAsMember` (a run parked through the page tools reads back through the token with `startedBy` the member), `D23.refreshRotates` (a new pair; the old refresh token → `invalid_grant`), `D23.narrowedConsent` (untick `workflow:run` + `workflow:files` → the token's `workflow.submit` is refused naming `workflow:run`; `workflow.status` still answers), `D23.revoke` (RFC 7009 → the next call is 401). Needs the member credentials and CE ≥ 0.4.52 (the release carrying `oauth_protected_resource`). Not part of `all` | one hello run, left waiting on its island |
| `driven` | driven runs (ADR-0006; apps#598): a `hello/driven` run started over the MCP endpoint and driven by the implementation's **own dispatched job**, never by the walk — `driven.startPending` (`workflow.start` mints the run id, asks the harness's `run/drive` rule to `repository_dispatch` the implementation's `workflow-drive.yml`, and answers `pending: true` before any row exists), `driven.parksOnTheForm` (the job writes the rows and parks the run on `ask/0/answer`, `hello/driven`'s undeclared form), `driven.leaseCleared` (the driver exited rather than holding the run open — the row's `leaseOwner` is null), `driven.submitDispatches` (`workflow.submitStep` writes the answer over the endpoint and re-dispatches: "a driver was dispatched to continue the run", `dispatched: true`), `driven.completes` (the second job drives the run to `succeeded` with the submitted note in its `text` output), `driven.startedByTheToken` (the endpoint ran as the caller, D23: the record's `startedBy` is the member the app token was minted for) — polling `workflow.status` every 10 s, 8 minutes per wait (an Actions cold start is ~1–2 minutes). It **blocks rather than fails** while either cross-repo half is missing: `errors.tool` naming `NO_DRIVER` (the implementation publishes no `driver.repo`) or `errors.workflow` (no `hello/driven`). Mints and revokes its own app token like `mcp`; `WORKFLOW_APP_TOKEN` skips the mint, the member credentials are still needed for `whoami` and the run record. Not part of `all` | two Actions jobs in the implementation repo, one hello run |
| `ownership` | run ownership, live (spec 11, D26/D27/D29; ADR-0007): member A starts a `hello/driven` run on the page (reusing `hello`'s navigation) and lets it finish over MCP (`ownership.aRunStarted`, `ownership.aRunFinished`, best-effort — nothing below needs it complete); then, as a second, fully independent browser session for member B — `D26.listDefaultsToMine` (`GET /api/workflow/runs?impl=hello&workflow=driven` as B excludes A's run), `D26.runGetIsNotFound` (`GET /api/workflow/run?id=` as B → 200 `{ run: null }`), `D26.runUpdateIsNotFound` (`POST /api/workflow/run/update` as B → 404), `D29.signIsNotFound` (`POST /api/workflow/files/sign` on a `runs/<A's>/…` path as B → 404), `D29.inputsStaySigned` (the workflow's member-wide `inputs/` area still signs for B — skipped with a note when A's run references no input file, the ordinary case for `hello/driven`), `D27.scopeAllIsForbiddenForAMember` (`?scope=all` as B → 403) or, when B holds the project owner/admin role, `D27.scopeAllListsAll` instead (asserted, with a note explaining the swap), `D27.scopeAllAsked` (as A, only when A holds the role: `?scope=all` lists a run started as B too, the plain list does not — otherwise a note), and `D26.mcpOutputsIsNotFound` (`workflow.outputs` over the MCP endpoint as B, on A's run, → `errors.runId: 'No such run'`, a token minted the way `mcp` mints one — `outputs` rather than `status` because `status` routes an unresolved run through `reply.ts`'s `pendingOr`, which answers a 200 `pending` snapshot for any id minted inside its 10-minute window). `WORKFLOW_EMAIL_2`/`WORKFLOW_PASSWORD_2` or `WORKFLOW_APP_TOKEN_2` is member B's login — a person-created prerequisite the walk cannot create for itself (spec 11 §Testing); its absence is a `block`, not a FAIL. `D26.mcpOutputsIsNotFound` additionally needs B to hold membership on the harness project (any role — a 403 "You are not a member of this project" while minting B's token is recorded as a note and the check skipped, never failed). The `D27.scopeAllListsAll`/`D27.scopeAllAsked` branches only run when one of the two members already holds the project's owner/admin role; on a harness where neither does, both assert the plain `D27.scopeAllIsForbiddenForAMember`/skipped-with-a-note forms instead. Not part of `all` | two hello runs (one may finish, over MCP; spends nothing metered) |
...
pnpm workflow-live:walk driven --harness https://workflow-mcp.j5s.dev --out /tmp/walk-driven      # ADR-0006; BLOCKED until hello publishes driven + a driver
WORKFLOW_EMAIL_2=b@example.com WORKFLOW_PASSWORD_2=... pnpm workflow-live:walk ownership --harness https://workflow.j5s.dev --out /tmp/walk-ownership   # spec 11; BLOCKED without a second member
...
`CAPTURE_FIXTURE_URL` (optional) — the recording URL `capture-url` starts with.
`WORKFLOW_EMAIL_2`/`WORKFLOW_PASSWORD_2` (or `WORKFLOW_APP_TOKEN_2`) is member B for the
`ownership` walk — a second account on the live harness, person-created, with **no CI
alias**: unlike `WORKFLOW_EMAIL`/`WORKFLOW_PASSWORD` it is a prerequisite the walk cannot
create for itself (spec 11 §Testing), and its absence blocks the walk rather than failing
a check. Two more `ownership` preconditions, both person-set on the harness rather than
env vars: member B must also be a member of the harness project (any role — viewer is
enough) for `D26.mcpOutputsIsNotFound` to mint its own token instead of noting a skip; and
`D27.scopeAllListsAll`/`D27.scopeAllAsked` only exercise when one of the two members
already holds the project's owner/admin role.
