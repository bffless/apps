...
const REGISTERED = ['m1', 'interactive', 'hello', 'headless', 'studio-audit', 'studio-headless', 'page-tools']

describe('WALKS', () => {
  it('registers every walk added so far', () => {
    for (const name of REGISTERED) expect(typeof WALKS[name]).toBe('function')
  })
  it('all runs the Task 25 walks in order, studio last — page-tools is not in it', () => {
