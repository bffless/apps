...
const workflowSchema: Record<string, any> = {
  ...
  "$defs": {
    ...
    "keep": {
      "description": "Retention for the workflow's runs (05): a whole number of hours or days, at least 1 (no 0d, no leading zeros), e.g. 30d. Absent means a run is never swept. Its own grammar, not `duration` — no ms/s/m, and `retry.delay: 30d` stays invalid.",
      "type": "string",
      "pattern": "^[1-9][0-9]*(h|d)$"
    },
    "level": {
