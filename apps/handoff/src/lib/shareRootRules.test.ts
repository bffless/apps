 ...
 * together; `has exactly 13 embedded folderChain functions` below pins that every gate still
 * carries it. The margin-comments gates (GET/POST /api/comments, spec 2026-07-28) joined the
 * set, bumping the copy count and the widened-query count below.
 *
...
describe('Task 5 — root sentinel resolved into the ACL folder chain', () => {
  ...
  it('has exactly 13 embedded folderChain functions', () => {
    // 8 original + the two feed select handlers (#188) + the PATCH
    // /api/node/meta gate (Task 5) + the GET/POST /api/comments gates
    // (margin comments), each a verbatim copy.
    expect(chainCodes.length).toBe(13)
  })
  ...
  it('widens every chain-feeding query (allFolders / folders) to nodeType in [folder, root]', () => {
    ...
    // 8 original + the GET/POST /api/comments gates' allFolders queries.
    expect(widened.length).toBe(10)
  })
