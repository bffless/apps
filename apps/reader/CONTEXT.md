...
- **items** — `id`, `guid` (dedup key), `feedId`, `title`, `link`, `author`, `publishedAt`, `summary`, `content`, `read` (boolean, default false), `starred` (boolean, default false), `fetchedAt` (**`number`, epoch-ms** — ingest stamps `Date.now()`; the retention prune filters it with a numeric `<`, so it must be numeric, not an ISO string — #119).
