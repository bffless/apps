...
- **items** — `id`, `guid` (dedup key), `feedId`, `title`, `link`, `author`, `publishedAt`, `summary`, `content`, `read` (default false), `starred` (default false), `fetchedAt`.
- **`data_upsert_many` mapping** (normalized entry → `items`, `dedupKey: guid`): `guid ← guid||link||hash` · `title, link, author, publishedAt, summary, content ← entry.*` · `feedId ← current feed` · `read/starred/fetchedAt ←` defaults.
