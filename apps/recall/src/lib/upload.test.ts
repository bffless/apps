...
function mockFetchSequence() {
  ...
  vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
    ...
    if (url.endsWith('/prepare')) {
      return new Response(JSON.stringify({ uploadUrl: 'https://bucket/put', storageKey: 'videos/v1/source/d/u-f.mov', originalName: 'f.mov' }), { status: 200 })
    }
    if (url.startsWith('https://bucket/')) return new Response(null, { status: 200 })
    if (url.endsWith('/register')) {
      return new Response(JSON.stringify({ url: '/api/uploads/videos/v1/source/d/u-f.mov' }), { status: 200 })
    }
...
describe('presignedUpload threads videoId', () => {
  it('sends videoId in BOTH the prepare and register bodies', async () => {
    const calls = mockFetchSequence()
    const file = new File([new Uint8Array([1, 2, 3])], 'f.mov', { type: 'video/quicktime' })
    const url = await presignedUpload(file, '/api/uploads/source', 'v1')
    expect(url).toBe('/api/uploads/videos/v1/source/d/u-f.mov')
    ...
    expect(prep.videoId).toBe('v1')
    expect(prep.filename).toBe('f.mov')
    expect(reg.videoId).toBe('v1')
    expect(reg.storageKey).toBe('videos/v1/source/d/u-f.mov')
  })

  it('throws when videoId is empty (defensive — uploads are always video-scoped)', async () => {
    mockFetchSequence()
    const file = new File([new Uint8Array([1])], 'f.mov', { type: 'video/quicktime' })
    await expect(presignedUpload(file, '/api/uploads/source', '')).rejects.toThrow(/videoId/)
  })
})

describe('isUploadServePath', () => {
  it('matches persisted relative /api/uploads/ serve paths', () => {
    expect(isUploadServePath('/api/uploads/audio/2026-06-12/x-original-77-79.wav')).toBe(true)
    expect(isUploadServePath('/api/uploads/source/2026-06-12/clip.mp4')).toBe(true)
  })
...
describe('presignedUpload', () => {
  ...
  it('runs prepare → PUT → register and returns the stored url', async () => {
    ...
    const url = await presignedUpload(file, '/api/uploads/source', 'video-1')
  ...
  it('reads url from nested record/data shapes', async () => {
    ...
    expect(await presignedUpload(file, '/api/uploads/audio', 'video-1')).toBe('https://cdn/nested')
  })

  it('throws when prepare fails', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(fail(403)))
    await expect(presignedUpload(file, '/api/uploads/source', 'video-1')).rejects.toThrow(
      /prepare failed \(403\)/,
    )
  })
  ...
  it('rethrows a reset that survives every retry as a clear message, not the raw NetworkError', async () => {
    ...
    await expect(
      presignedUpload(file, '/api/uploads/source', 'video-1', {
        maxPutAttempts: 2,
        backoff: () => Promise.resolve(),
      }),
