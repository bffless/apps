 ...
 * Recall's proxy rules key both `subDir`s off `request.body.videoId` (not a
 * project id — Recall is per-video, not per-project), so `presignedUpload`
 * threads a `videoId` through the prepare and register bodies.
 *
 * NOTE: the bucket must allow PUT from the site origin (CORS) or the browser
 * blocks step 2. The recall upload routes require auth — an unauthenticated
 * `POST ${basePath}/prepare` returns 401; `fetchWithReauth` refreshes the
 * session on a 401 before retrying.
 *
 * Step 2 is a single-shot `PUT` of the whole file over one TCP stream. A flaky
 * uplink can reset that stream mid-flight (Firefox: `NS_ERROR_NET_RESET`, 0 B
...
/** The two upload kinds Recall has: the source video and its extracted audio. */
export type UploadKind = 'source' | 'audio'
 ...
 * `/api/uploads/source/{prepare,register}` rule `maxFileSize` in sync.
 */
...
async function mintUploadUrl(basePath: string, file: File, videoId: string): Promise<MintedUpload> {
  const prepRes = await fetchWithReauth(`${basePath}/prepare`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ filename: file.name, videoId }),
  })
 ...
 * (e.g. `/api/uploads/source`). `videoId` is sent to the prepare and
 * register rules, which interpolate it into the per-video storage key
 * (`videos/<videoId>/<type>/...`). Throws with a descriptive message if any
 * step fails.
 */
export async function presignedUpload(
  ...
  if (!videoId) throw new Error('presignedUpload: videoId is required')
  ...
  for (let attempt = 1; attempt <= maxPutAttempts; attempt++) {
    const minted = await mintUploadUrl(basePath, file, videoId)
    let putRes: Response
    try {
