...
| D23 | Auth ladder: authless dev prototype → CE user-bound scoped app tokens → OAuth 2.1 (access token = app token); `.well-known` ships as a rule; per-rule `requiredScopes` enforced by `auth_required`, effective permission = member's ∩ token's | 10 |
