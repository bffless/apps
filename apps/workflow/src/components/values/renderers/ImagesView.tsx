 ...
 * to point a `src` at — an `image/*` ref whose `url` is same-origin
 * (`isSameOriginUrl`, Task 15's P1 ruling; the stricter media-sink gate, not
 * `FileCard`'s own `isSafeUrl`, because this component builds the `src`
 * itself rather than trusting a writer-supplied player choice).
 *
 ...
 */
import { downloadHref, isSameOriginUrl } from '../../../lib/url'
...
function ImageItem({ fileRef }: { fileRef: FileRef }) {
  const isImage = fileRef.contentType?.startsWith('image/') && isSameOriginUrl(fileRef.url)
