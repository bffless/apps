...
describe('setHeadless — the page\'s Accept (07, apps#432)', () => {
  it('re-sends bffless.headless over host-context-changed to a mounted island', async () => {
    const h = await mounted()
    expect(h.island.app.getHostContext()).toMatchObject({ bffless: { headless: false } })

    h.host.setHeadless(true)
    await tick()

    expect(h.island.contextChanges.at(-1)).toEqual({ bffless: { headless: true } })
    // The View merged it: an island reading `getHostContext().bffless` sees the flag.
    expect(h.island.app.getHostContext()).toMatchObject({ bffless: { headless: true } })
    // Nothing else in the context was disturbed.
    expect(h.island.app.getHostContext()?.displayMode).toBe('inline')
  })

  it('applies a flag set before the bridge connects through ui/initialize', async () => {
    const h = makeHarness()
    const mounting = h.host.mount(h.iframe, MOUNT)
    h.host.setHeadless(true)
    await h.island.connect()
  ...
  it('is a no-op when unchanged, before a mount, and after teardown', async () => {
    const before = makeHarness()
    expect(() => before.host.setHeadless(true)).not.toThrow()
