 ...
 * inlines into *both* HTML files while deleting it from the bundle. So the
 * stager runs this config once per island with `WORKFLOW_ISLAND` set, and the
 * root is that island's own directory — which also means the emitted file is
 * plain `index.html` and the flattening below is a one-line rename rather than
 * a fight with Rollup's output naming.
 *
 * Env (both set by `scripts/stage-hello.mjs`):
 * - `WORKFLOW_ISLAND`      the directory under `hello/islands/` to build (required)
 * - `WORKFLOW_ISLANDS_OUT` where `<island>.html` lands (default `hello-dist/islands`)
 */
...
if (!island || !/^[a-z0-9-]+$/.test(island)) {
  throw new Error(
    `vite.islands.config.ts: set WORKFLOW_ISLAND to an island directory under hello/islands/ (got ${String(island)})`,
  )
}

const outDir = process.env.WORKFLOW_ISLANDS_OUT ?? resolve(here, '../hello-dist/islands')
