...
${end.comment}` : end.comment;
  ...
  // src/lib/runner/graph.ts
  function declaredNeeds(def, job) {
  ...
  function topoLayers(def) {
    ...
    while (remaining.length > 0) {
      const layer = remaining.filter((id) => declaredNeeds(def, id).every((n) => placed.has(n))).sort();
      if (layer.length === 0) {
  ...
  function describeInput(decl) {
    const input = { type: typeof decl.type === "string" ? decl.type : "string" };
    if (decl.list === true) input.list = true;
    if (decl.required === true) input.required = true;
    if (decl.default !== void 0) input.default = decl.default;
    if (decl.options !== void 0) input.options = decl.options;
    return input;
  }
  function describeOutput(decl) {
    if (!isPlainObject2(decl)) return {};
    const output = {};
  ...
  function describeStep2(step) {
    ...
    if (step.uses === "island" || step.uses === "form") {
      ...
      if (mode !== void 0) described.headless = mode;
      if (typeof withDecl.title === "string" && withDecl.title !== "") described.title = withDecl.title;
    }
    if (step.uses === "island" && isPlainObject2(raw.outputs)) described.outputs = raw.outputs;
    if (step.uses === "form" && isPlainObject2(withDecl.fields)) described.fields = withDecl.fields;
    return described;
  }
  function orderedJobs(def) {
    try {
      return jobOrder(def);
    } catch {
  ...
  function describeWorkflow(a) {
    const raw = isPlainObject2(a.def.raw) ? a.def.raw : {};
    const inputs = {};
    for (const [name, decl] of Object.entries(a.def.inputs)) inputs[name] = describeInput(decl);
    const outputs2 = {};
    for (const [name, decl] of Object.entries(a.def.outputs)) outputs2[name] = describeOutput(decl);
    ...
    const description = typeof raw.description === "string" && raw.description !== "" ? raw.description : void 0;
    return {
      impl: a.impl,
      workflow: a.workflow,
      name: a.def.name,
  ...
  // src/mcp/csp.ts
  function uiMeta(appOrigin, storageOrigin2) {
    const connect = [appOrigin, storageOrigin2].filter((origin) => origin !== "");
    const resource = storageOrigin2 === "" ? [] : [storageOrigin2];
  ...
  var RESOURCE_MIME = "text/html;profile=mcp-app";
  ...
  function implementationOf(alias, step) {
  ...
  function discovered(steps) {
    const plan = steps.plan;
    if (!plan) return [];
    const fetched = [steps.index1, steps.index2, steps.index3];
    return plan.aliases.map((alias, i) => implementationOf(alias, fetched[i])).filter((impl) => impl !== null);
  }
  function listText(implementations) {
  ...
  function list(route, steps) {
    const only = route.impl === "" ? void 0 : route.impl;
    if (only === void 0 && steps.aliases?.ok !== true) return refuse("discovery", REFUSALS.discovery);
    const implementations = discovered(steps).map(({ islands, ...impl }) => {
      void islands;
      return impl;
    });
    if (only !== void 0 && implementations.length === 0) return refuse("impl", `No implementation "${only}" is published here`);
...
${step.key} is a form step; in this phase it is completed on the harness page.`
  ...
  function outputs(route, steps) {
    ...
    const text = names.length === 0 ? `Run ${runId} is ${runStatus} and has no outputs${runStatus === "running" ? " yet" : ""}` : `Run ${runId} (${runStatus}) outputs: ${names.join(", ")}`;
  ...
  function runs(route, steps) {
    ...
    const listed = runsWithWaiting(steps.runs, steps.waiting).filter((row) => typeof row.runId === "string" && typeof row.status === "string").filter((row) => wanted === void 0 || row.status === wanted).sort((a, b) => (typeof b.startedAt === "number" ? b.startedAt : 0) - (typeof a.startedAt === "number" ? a.startedAt : 0)).slice(0, limit).map((row) => ({
      ...
      ...row.unattended === void 0 || row.unattended === null ? {} : { unattended: row.unattended === true },
...
${lines.join("\n")}`,
  ...
  function storageOrigin(steps) {
    return originOf(steps.probe?.url);
  }
  function resourcesList(route, steps) {
  ...
  function resourcesRead(route, steps, id) {
    const source = route.isStepView ? steps.stepView : route.isIslandUri ? steps.island : void 0;
  ...
  function handler(data) {
    ...
    const json = (value) => ({ json: JSON.stringify(value), status: 200 });
