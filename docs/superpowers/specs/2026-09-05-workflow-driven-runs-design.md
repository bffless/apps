...
- CE's `github_api` handler has a `dispatch` operation (`workflow_dispatch` and
  `repository_dispatch`; `apps/backend/src/pipelines/handlers/github-api.handler.ts`).
...
| DR6 | **`drive` is a harness rule.** `POST /api/workflow/run/drive { id }`, in the harness rule set, refuses on a terminal run or a live lease, then `github_api` dispatches the implementation's driver workflow. The GitHub token is a project secret. |
| DR7 | **The implementation declares its driver in `index.json`**, generated by its CI (`driver: { repo, workflow }`), never hand-written. No `driver` ⇒ `drive` refuses with `NO_DRIVER` and `workflow.start` over the endpoint stays not served for that implementation. |
...
`POST /api/workflow/run/drive` `{ id }` — harness rule set, `auth_required`, scope `workflow:run`.

Pipeline: `data_query` the run row → `function_handler` decides → `github_api` dispatch →
`response`.
...
| `DISPATCH_FAILED` | GitHub answered non-2xx; the body is echoed |

Success: `202 { dispatched: true, repo, workflow, ref: 'main' }`.

The implementation's `index.json` is read the way discovery reads it
(`/w/<alias>/.bffless/workflows/index.json`, spec 06); `driver` is
`{ "repo": "<owner>/<name>", "workflow": "workflow-drive.yml" }`, filled in by the publish
step from `github.repository` and a fixed file name. The GitHub token is the
project secret `GITHUB_DRIVER_TOKEN` (fine-grained, `actions: write` on the implementation
repos); `github_api` reads it through the pipeline's secrets context, so it never reaches a
browser. The dispatch inputs are `{ mode: 'resume', run_id, harness_url }` or
`{ mode: 'run', run_id, workflow, inputs, harness_url }`.
...
(`bffless/publish-workflow`, and `@bffless/workflow publish`) from `github.repository`. Modelled on `.github/workflows/workflow-headless-run.yml`:

```yaml
on:
  workflow_dispatch:
    inputs:
      mode:        { type: choice, options: [run, resume], required: true }
      run_id:      { type: string, required: true }
      harness_url: { type: string, required: true }
      workflow:    { type: string, required: false }   # run only: <impl>/<workflow>
      inputs:      { type: string, required: false }   # run only: JSON object
concurrency: { group: "drive-${{ inputs.run_id }}", cancel-in-progress: false }
```
...
- The GitHub token is a project secret. It is the one credential that must be provisioned
  by hand per instance (`install-app` verifies, never provisions, per its own contract).
...
| what | behaviour |
|---|---|
| dispatch accepted, job never starts (Actions outage, bad token at run time) | `workflow.status` reads `pending` for 10 min, then `RUN_NOT_FOUND`; nothing was written. The agent re-issues `start`. |
