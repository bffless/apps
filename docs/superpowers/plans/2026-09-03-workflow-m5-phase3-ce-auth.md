...
## Phase 3 as shipped (2026-09-04)

**CE** landed on `main` as ce#730 (story 7, v0.4.43), ce#731 (story 8, v0.4.44), ce#734 (story 9, v0.4.45), with ce#733 (throttle-map bound that missed #730's merge), ce#737 (the invoker parses a sibling's ≥ 256 KiB passthrough-string body — `workflow.stepView`'s island), ce#739 (issuer = `https://<ADMIN_DOMAIN>` or `OAUTH_ISSUER`, never `FRONTEND_URL` — v0.4.46), ce#742 (lenient RFC 7591 registration; claude.ai's body carried metadata the global validation refused — v0.4.47), ce#744 (a config panel for `mcp_handler`), and ce#746 + ce#747 (v0.4.48: the nginx reload watcher lost inotify events during the startup config burst, leaving `workflow.j5s.dev` on the wildcard vhost; now a coalescing watcher, atomic rename-into-place writes, in-place regeneration, a boot guard, and a wildcard handler that resolves a mapped host from its mapping). `bffless` CLI 0.3.6 carries `bypassVisibility`; `deploy-proxy-rules` v1.3.1 bundles it (released with a `Release-As` commit — the bump was a `chore`). **Apps:** #583 (story 7), #584 (story 8), #585 (story 9), stacked on the epic.

**Departures, all recorded on the PRs:** the story-8 invoker builds an `external_proxy` sibling's headers through `buildProxyHeaders` (the rule's `forwardCookies`/`authTransform`/`headerConfig`), so the `/w/<alias>/*` forwarder now lists `authorization` in `headerConfig.forward` (workflow-cli `prepare`; the live `bffless/workflow` hello forwarder was patched by hand until the implementations repo redeploys with the released CLI); the step-view resource is `ui://bffless/workflow/step-view.html` (claude.ai caches a widget's resource per URI on the connector and had pinned CE's not-found page from the outage — #587 hashes the URIs by build); the DCR spike's decision stands (built-in), and CIMD — claude.ai's *recommended* connector mode — is ce#741.

**Gate:** claude.ai's connector completed DCR → consent → tokens against the private `workflow.j5s.dev`; status/runs ran as the member; the island mounted, was completed and submitted in the chat; a `workflow:read`-only consent could not submit while status still answered; App Tokens listed and revoked the OAuth tokens (screenshots on #585, three comments). Walks: `mcp` 26/26 on both hosts, `oauth` 9/9 on both (`workflow.j5s.dev` had never been walked with `mcp` before; that is what surfaced the forwarder and the island 401). Timings: the `mcp` walk ~1m35s on the function endpoint, ~1m45s on `mcp_handler` — no per-call regression.

**Follow-ups filed:** apps#586 (an `mcp-app` walk emulating the agent host), apps#587 (build-hashed resource URIs), apps#588 (a session from an app token for the headless driver), apps#589 (CE advertises its issuer), ce#738 (in-process sibling calls persist no debug logs), ce#741 (CIMD), platform#195 (the k8s admin vhost's `/.well-known/oauth-authorization-server` location).
