...
Verified: `mcp` walk **24/24** on `https://workflow-mcp.j5s.dev` (Story 5's 13 + Story 6's 11); the spike's (a)/(b) on #554 with evidence, (c) up to the wire (the official SDK client), the connector UI itself awaiting the person's screenshot. Scratch project kept, named, documented (`apps/workflow/bffless/README.md`).
