...
2. **⚑ CONFIRMED — Frames come from two new CE `ffmpeg_handler` ops, `frames` and `contact_sheet`.** Spec 03/06 assumed `scripts/contact-sheet.js`; a Worker has no `<video>`/canvas, so that assumption was wrong. `frames` = one fast-seek still per requested time (`-ss T -i in -frames:v 1`), `contact_sheet` = the same stills with the clock burned in (`drawtext`) tiled `columns×rows` per sheet — Studio's `contactSheet.ts` planning constants (5–30 s interval, 3 columns, 9–12 cells, ≤10 sheets) become the op's defaults. Both are path-in/path-out under an explicit output prefix, so they also serve the blog's frame re-capture (Studio ADR-0002 "recapture, never crop"). A CE PR (Phase 4, Task 27) is on the critical path; `drawtext` needs an ffmpeg with libfreetype — the Debian `ffmpeg` on the droplet and in `workers/ffmpeg/Dockerfile` has it, and the op degrades to un-labelled cells with a warning when the filter is missing (live-walk check).
...
- **PR titles are release commits** (squash + release-please): CE `feat(cli): --path-prefix for rules build, push and diff`; deploy-proxy-rules `feat: path-prefix input`; apps Phase 1 `feat(workflow-lint): publish the package, add the index verb and --path-prefix`; apps Phase 2 `refactor(workflow): hello lives in bffless/workflow-hello`; apps 3a/3b per Decision 20; CE Phase 4 `feat(pipelines): ffmpeg_handler frames and contact_sheet ops`; apps Phase 4 `feat(workflow-studio): the Studio port`. Push every commit before opening a PR; re-check merge state before follow-ups (the user merges fast).
...
### Task 17: CE `ffmpeg_handler` ops `frames` and `contact_sheet` (Decision 2)

**Files (repos/ce):**
- Modify: `apps/backend/src/pipelines/ffmpeg/ffmpeg-args.ts` (+ `buildFrameArgs`, `buildTileArgs`), `apps/backend/src/pipelines/ffmpeg/ffmpeg-args.spec.ts`, `apps/backend/src/pipelines/handlers/ffmpeg.handler.ts:34,74-93,132-141` (+ `runFrames`, `runContactSheet`, `resolveTimes`), `apps/backend/src/pipelines/handlers/ffmpeg.handler.spec.ts`, `apps/backend/src/pipelines/execution/step-handler.interface.ts:664-738` (`FfmpegOperation`, config fields, TSDoc), `apps/backend/src/pipelines/ffmpeg/ffmpeg-capability.service.ts:12` (`FFMPEG_OPS` + a `drawtext` filter probe), `apps/backend/src/mcp/tools/proxy-rules.tools.ts:69` (the describe bullet), `apps/frontend/src/components/pipelines/handlers/types.ts:464-480`, `apps/frontend/src/components/pipelines/handlers/FfmpegHandlerConfig.tsx:21-51` (two entries + `FIELDS_BY_OPERATION`)
- Docs: `repos/docs/docs/features/server-video-ops.md` ("four curated operations" → six, with the two new tables), `repos/skills/plugins/bffless/skills/pipelines/SKILL.md` (add the `ffmpeg_handler` row the table lacks)

**Interfaces (config):**
- `frames`: `input` (template), `times: number[] | string` (expression-evaluated like `spans`), `outputPrefix` (template, uploads-relative dir), `height?` (default 720), `quality?` (jpeg q, default 3). Output: `{ frames: [{ time, storage_path, content_type: 'image/jpeg', size }], count, …telemetry }`. Files: `<outputPrefix>/frame-<NN>.jpg` (1-based, zero-padded to 2).
- `contact_sheet`: `input`, `outputPrefix`, `duration?` (number|expression; probed with ffprobe when absent — one extra `ffprobe` command in the same job), `interval?` (seconds; default = Studio's plan: `clamp(duration / MAX_FRAMES, 5, 30)`), `columns?` (default 3), `cellsPerSheet?` (default 9, max 12), `maxSheets?` (default 10), `height?` (default 720), `label?` (default `true`: burn `m:ss` into the corner). Output: `{ sheets: [{ storage_path, content_type: 'image/jpeg', size, times: number[], index, total, cols, rows }], interval, count, labelled: boolean, …telemetry }`. Files: `<outputPrefix>/sheet-<NN>.jpg` plus scratch `cell-<NNN>.jpg`.

- [ ] **Step 1: Failing tests**

```ts
// ffmpeg-args.spec.ts (append)
describe('buildFrameArgs', () => {
  it('fast-seeks to the time and writes one labelled jpeg', () => {
    const args = buildFrameArgs({ input: 'in.mp4', output: 'frame-01.jpg', time: 83.5, height: 720, quality: 3, label: '1:23' })
  ...
  it('omits drawtext without a label', () => { expect(argAfter(buildFrameArgs({ …, label: undefined }), '-vf')).not.toContain('drawtext') })
})
describe('buildTileArgs', () => {
  it('tiles a numbered sequence into one sheet', () => {
    expect(buildTileArgs({ pattern: 'cell-%03d.jpg', start: 1, count: 9, columns: 3, output: 'sheet-01.jpg' })).toEqual([
      '-start_number', '1', '-i', 'cell-%03d.jpg', '-frames:v', '1', '-vf', 'tile=3x3:padding=2:margin=2:color=0x111111', '-q:v', '3', 'sheet-01.jpg',
    ])
  })
})
```
```ts
// ffmpeg.handler.spec.ts (append; the file already fakes the executor and asserts the FfmpegJob shape)
it('frames: one command per time, outputs named frame-NN under outputPrefix', …)   // 3 times → 3 ffmpeg commands, 3 outputs, keys `<uploads>/<prefix>/frame-01.jpg`…
it('contact_sheet: plans Studio-style, emits cells then one tile per sheet, returns times per sheet', …)   // duration 600 → interval 5 → 120 frames → 10 sheets × 12 cells (cap) ⇒ 120 cell commands + 10 tile commands, outputs = 10 sheets
it('contact_sheet: label:false skips drawtext; a missing drawtext filter (capability) falls back with labelled:false', …)
it('validateConfig requires input + outputPrefix (+ times for frames)', …)
```

- [ ] **Step 2: Verify fail** — `cd apps/backend && npx jest ffmpeg` → FAIL.
- [ ] **Step 3: Implement**

```ts
// ffmpeg-args.ts (append)
/** One still: input seek (fast, keyframe-accurate enough for a contact sheet), scaled, optionally labelled. */
export function buildFrameArgs(o: { input: string; output: string; time: number; height: number; quality: number; label?: string }): string[] {
  const filters = [`scale=-2:${o.height}`]
  if (o.label !== undefined) {
    // drawtext needs libfreetype; the handler only passes a label when the capability probe saw the filter.
    const text = o.label.replace(/\\/g, '\\\\').replace(/:/g, '\\:').replace(/'/g, "\\'")
    filters.push(`drawtext=text='${text}':fontsize=h/12:fontcolor=white:box=1:boxcolor=black@0.6:boxborderw=8:x=w-tw-16:y=h-th-16`)
  }
  return ['-ss', secs(o.time), '-i', o.input, '-frames:v', '1', '-vf', filters.join(','), '-q:v', String(o.quality), o.output]
}
/** Tile `count` numbered cells into one sheet (the last sheet may be short — tile pads with the color). */
export function buildTileArgs(o: { pattern: string; start: number; count: number; columns: number; output: string }): string[] {
  const rows = Math.ceil(o.count / o.columns)
  return ['-start_number', String(o.start), '-i', o.pattern, '-frames:v', '1', '-vf', `tile=${o.columns}x${rows}:padding=2:margin=2:color=0x111111`, '-q:v', '3', o.output]
}
export function clockLabel(seconds: number): string { const s = Math.max(0, Math.round(seconds)); return `${Math.floor(s / 60)}:${String(s % 60).padStart(2, '0')}` }
```
Handler: `OPERATIONS = ['probe', 'extract_audio', 'slice', 'concat', 'frames', 'contact_sheet']`; `validateConfig` needs `input`+`outputPrefix` for both and `times` for `frames`; `resolveTimes(config.times, …)` mirrors `resolveSpans` (array or expression → finite non-negative numbers, `INVALID_TIMES`). `runFrames`: `inputKey = resolveKey(config.input, …, 'input')`, `prefix = resolveKey(config.outputPrefix, …, 'output')` (a dir: strip a trailing `/`), one `FfmpegJobCommand` per time (`{in:inNAME}` → `{out:frame-NN.jpg}`), outputs listed, result as above. `runContactSheet`: `duration` from config or a first `ffprobe` command (`buildProbeArgs`, parse `format.duration` from `res.stdout` — the executor returns the *last* command's stdout, so run the probe as its **own** job first, then the frames job); plan with Studio's rules — `interval = clamp(duration / (maxSheets * cellsPerSheet), 5, 30)`, `times = [interval/2, interval/2 + interval, …] < duration`, `perSheet = min(cellsPerSheet, ceil(times.length / maxSheets))` — cells `cell-<NNN>.jpg` (scratch-only: **not** in `outputs`, so the executor never uploads them; both executors already keep unlisted `{out:}` files in scratch), then one `buildTileArgs` per sheet with `start`/`count`; outputs `sheet-<NN>.jpg`. `label` → `clockLabel(time)` when `config.label !== false && capability.hasFilter('drawtext')`; the capability service's `probe()` additionally runs `ffmpeg -hide_banner -filters` once and remembers whether `drawtext` is listed (`hasFilter`), and the probe payload gains `filters: { drawtext: boolean }`. Remote executor: unchanged (ADR-0004 — argv runner); the remote Worker's `ffmpeg` is Debian's (libfreetype present), but `labelled` is reported from the *local* probe — document it, and let the op fall back to `label:false` on `FFMPEG_FAILED` whose stderr mentions `drawtext` (one retry without labels, `labelled:false`).
- [ ] **Step 4: Verify** — `npx jest ffmpeg`; `pnpm --filter backend build`; frontend `pnpm --filter frontend lint`; a local smoke against `pnpm dev:full` with a 10-second fixture (`ffmpeg -f lavfi -i testsrc=duration=10:size=320x240:rate=25 /tmp/…/t.mp4`): a rule with `contact_sheet` returns one sheet with 2 labelled cells.
- [ ] **Step 5: Commit + PR + release** — `feat(pipelines): ffmpeg_handler frames and contact_sheet ops`; after merge release-please cuts the next CE minor; **j5s must run it** (ask the user to upgrade j5s once released — the port's live walk needs the ops; `probe.ops` on `GET /api/video/capabilities` lists them).
