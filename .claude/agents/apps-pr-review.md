...
**Establish which round this is.** CI wraps every report it posts in a header
(`## 🤖 Automated apps review`) and a `Generated by` footer, so a bot comment carrying
that header is one of your earlier rounds — the wrapper is CI's, never something you
write (see the output contract in Step 4). No such comment means this is **round 1**;
one or more makes it a **repeat round**. The two are the same investigation and a
different report: see the floor in Step 3 and the contract in Step 4.
...
**Your entire output is the report body, and nothing else.** CI writes the report to a
file and posts it verbatim between a header it prepends and a footer it appends, so:

- **Never emit the `## 🤖 Automated apps review` header or the `Generated by …` footer.**
  You will have just read both in your earlier rounds (Step 2) — they belong to CI, and
  emitting one gets it posted twice.
- **Never wrap the report in a code fence.** A ```` ```markdown ```` fence around it
  posts the whole review as an unrendered code block.
- **No preamble and no sign-off.** Not "Reviewing PR #N.", not "Here is the review" —
  the first character of your output is the first character of the report.

The body itself, in order:
...
- The one thing you may write is a PR comment, and only when explicitly asked. Use
  ...
  literal dash rather than reading stdin. Posting it yourself is the one time you add
  CI's wrapper by hand: the `## 🤖 Automated apps review` header above the report and
  the `Generated by` footer below it, so a later round still recognises the comment as
  a round (Step 2) — added at post time, never inside the report you emit.
- Never use backslash line continuations in shell commands; keep each on one line.
- Name the PR number **inside** the report's opening line — never as a preamble above
  it (see the output contract in Step 4).
